<template>
  <v-container grid-list-md>
    ...
    <div class="text-xs-center display-2 font-weight-light text-capitalize">{{$route.params.city}} Schools</div>
    <v-layout row wrap>
        ...
        <div style="margin:10px;">
          <v-hover>
            <v-card hover slot-scope="{ hover }" :to="goto(school.name)" class="mx-auto" dark>
              <v-img style="padding-top:50%;max-height:250px;min-height:250px;" :src="images[index%6]">
                ...
                <v-expand-transition>
                  <div v-if="hover" class="d-flex transition-fast-in-fast-out black darken-2 v-card--reveal title font-weight-light white--text px-3" style="height: 100%; text-align:center;">
                    <div>
      ...
      <v-flex @click="addFormVisible=true" style="
      height: 250px;
      width: 250px;
      " xs12 sm6 md4>
        <v-card hover style="
          ...
          <v-form >
            <v-container>
              <v-layout row wrap>
                ...
                <v-flex xs12 sm6>
                  <v-text-field v-model="newSchool.principalname" label="Name of the Principle" required></v-text-field>
                </v-flex>
                <v-flex xs12 sm6>
                  <v-text-field v-model="newSchool.contact" label="Contact No. of The School" required></v-text-field>
                </v-flex>
  
              </v-layout>
            </v-container>
            <v-btn @click.prevent="addSchool" 
  ...
  export default {
    ...
    methods: {
      ...
      addSchool() {
        ...
        this.$store.dispatch("city/addSchool",{data: this.newSchool,city:this.$route.params.city}).then(
          ()=>{
            this.snackbar=true
