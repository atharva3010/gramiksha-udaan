  ...
  <v-container>
    ...
    <v-layout>
      <v-flex sx12 sm6 offset-sm3>
        <v-card>
          <v-container>
            <h1 class="font-weight-light display-1">Add Volunteer</h1>
            <v-form>
              ...
              <v-btn @click="addUser" :disabled="loading" :loading="loading">Add Volunteer</v-btn>
            </v-form>
          </v-container>
        </v-card>
      </v-flex>
...
export default {
  ...
  methods: {
    addUser() {
      ...
      this.message = "";
      this.$store
        .dispatch("user/addUser", {email:this.email,post:"vol"})
        .then(() => {
          this.loading = false;
          this.snackbar = true;
          this.email = "";
